...
def filter_boxes(resnet_output, obj_threshold, anchors=ANCHORS):

    grid_h, grid_w, nb_box = resnet_output.shape[:3]
    resnet_output[..., 5:] = softmax(resnet_output[..., 5:]) * sigmoid(resnet_output[..., 4][..., np.newaxis])
    resnet_output[..., 5:] *= resnet_output[..., 5:] > obj_threshold
    ...
    for row in range(grid_h):
        for col in range(grid_w):
            for b in range(nb_box):
                ...
                classes = resnet_output[row, col, b, 5:]

                if np.sum(classes) > 0:
                    # first 4 elements are x, y, w, and h
                    x, y, w, h = resnet_output[row, col, b, :4]
